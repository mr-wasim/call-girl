...
/* ================= PAGE ================= */
export default function ListingDetailPage({ data, error }) {
  const [openImg, setOpenImg] = useState(null)
  const [tab, setTab] = useState("reviews")

  if (error || !data) {
    return <div className="px-6 py-12 text-white">Profile not found</div>
  }
    ...
    <div className="bg-[#2b2b2b] text-[#d1d1d1] min-h-screen">
      ...
      <div className="bg-[#262626] border-b border-[#3a3a3a]">
        <div className="max-w-7xl mx-auto px-4 py-2 flex flex-wrap gap-4 text-sm">
          ...
          <a
            href={`https://wa.me/${onlyDigits(data.contact)}`}
            target="_blank"
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-5 grid grid-cols-2 gap-4">
          {images.map((img, i) => (
            <button key={i} onClick={() => setOpenImg(img)} className="relative">
              <img
                src={img}
                className="w-full h-[220px] sm:h-[260px] object-cover rounded"
                alt="profile"
              />
      ...
      {openImg && (
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-50">
          ...
          <img src={openImg} className="max-h-[90vh] max-w-[90vw] object-contain" alt="preview" />
        </div>
      )}
    </div>
  )
...
export async function getServerSideProps(context) {
  ...
  try {
    ...
    if (!json.success) {
      return { props: { data: null, error: "Not found" } }
    }
